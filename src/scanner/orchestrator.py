...
def _execute_auto_healing(merchant_name: str, url: str, debug_info: str) -> str:
    ...
    1. Si possible, génère uniquement une fonction python `_check_{merchant_name.lower()}
    (url: str, html: str) -> ScanResult | None:` pour 'analyzer.py'.
...
def _deploy_pull_request_healing(merchant_name: str, code_proposal: str) -> str | None:
    ...
    try:
        ...
        new_content = (
            file_content.decoded_content.decode("utf-8") + f"\n\n# Auto-Healing {merchant_name}\n{code_proposal}"
        )
