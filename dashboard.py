...
try:
    ...
    with c1:
        ...
        fig_status = px.pie(df, names='status_code', hole=0.4, color_discrete_sequence=px.colors.qualitative.Safe)
    ...
    # Filtre interactif
    status_filter = st.multiselect("Filtrer par statut", options=df['status_code'].unique(), default=df['status_code'].unique())
    filtered_df = df[df['status_code'].isin(status_filter)]
    
    st.dataframe(
        filtered_df,
        column_config={
            "url_wetall": st.column_config.LinkColumn("Page Wetall"), # Colonne cliquable
            "url_marchand_finale": st.column_config.LinkColumn("Lien Marchand"),
            "date_scan": st.column_config.DatetimeColumn("Date", format="DD/MM/YY HH:mm"),
        },
